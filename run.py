...
class DataRunner:
    ...
    def run_pre(
        ...
        @parmeter
        :param func_name: 如`index_tick`代表处理指数tick, `mayjor_tick`代表主力合约tick,\
    ...
    def run_after(
        ...
        kind: Literal["index", "mayjor", "secondery", "option"],
    ):
        """
        处理1m数据需要现有tick数据, 处理1d数据需要先有1m数据. 因此运行该函数之前, 需确保有相应的数据.
        @parmeter
        :param func_name: `future_1m`代表处理期货的1分钟线数据, 其他类推.
        :param kind: 期货数据的类别, 如`index`代表指数, `mayjor`代表主力合约, `secondery`代表次主力合约.\
                    如果第一个参数是`option_1d`, 则此参数无效, 仅代表日志名
        """
        try:
            func = FUNC_MAP[func_name]
            logger.add(
    ...
    def run_all_pre():
        ...
        with multiprocessing.Pool(processes=num_processes) as pool:
            pool.apply_async(DataRunner.run_pre, args=("index_tick",))
            pool.apply_async(DataRunner.run_pre, args=("mayjor_tick",))
            pool.apply_async(DataRunner.run_pre, args=("secondery_tick",))
            pool.apply_async(DataRunner.run_pre, args=("option_1m",))
            pool.close()
            pool.join()
    ...
    def run_all_1m():
        ...
        num_processes = multiprocessing.cpu_count()
        with multiprocessing.Pool(processes=num_processes) as pool:
            pool.apply_async(DataRunner.run_after, args=("future_1m", "index"))
            pool.apply_async(DataRunner.run_after, args=("future_1m", "mayjor"))
            pool.apply_async(DataRunner.run_after, args=("future_1m", "secondery"))
            pool.close()
            pool.join()
    ...
    def run_all_1d():
        ...
        num_processes = multiprocessing.cpu_count()
        with multiprocessing.Pool(processes=num_processes) as pool:
            pool.apply_async(DataRunner.run_after, args=("futrue_1d", "index"))
            pool.apply_async(DataRunner.run_after, args=("future_1d", "mayjor"))
            pool.apply_async(DataRunner.run_after, args=("future_1d", "secondery"))
            pool.apply_async(DataRunner.run_after, args=("option_1d", "option"))
            pool.close()
            pool.join()
    ...
    def run_data_supplement():
        ...
        DataRunner.run_all_pre()
        DataRunner.run_all_1m()
        DataRunner.run_all_1d()
